    ...
    MixinNonFungibleToken
{
    ...
    /// @return flag           True if the operator is approved, false if not
    function isApprovedForAll(address owner, address operator) override external view returns (bool flag) {
    ...
    function balanceOf(address owner, uint256 id) override external view returns (uint256 balance) {
    ...
    function balanceOfBatch(address[] calldata owners, uint256[] calldata ids) override external view returns (uint256[] memory balances_) {
        // sanity check
        require(
